...
/**
 * This MultiBrotSet is the result of a mathematical function, of which the complex numbers c are
 * inserted into f(z) = z^3 + c. If the function with point c does not diverge to infinity, c belongs to the
 * set.
 */
public class MultibrotSet extends Fractal {

    public MultibrotSet(int width, int height, double zoom, int maxIteration) {
     ...
     * bMandelbrot (power 3) function, the calcSet method is used.
    ...
    public int calcSet(double xCvalue,double yCvalue) {
        ...
        while (iterCount < maxIteration) {
            xZtemp = (xZvalue * xZvalue * xZvalue) - (3 * xZvalue * yZvalue * yZvalue) + xCvalue;
            yZvalue = 3 * (xZvalue * xZvalue * yZvalue) - (yZvalue * yZvalue * yZvalue) + yCvalue;
            xZvalue = xZtemp;
