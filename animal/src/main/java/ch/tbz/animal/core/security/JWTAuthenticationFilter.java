...
import ch.tbz.animal.core.security.helpers.Credentials;
import ch.tbz.animal.core.security.helpers.JwtProperties;
import ch.tbz.animal.domain.user.UserDetailsImpl;
import ch.tbz.animal.domain.user.dto.UserMapperImpl;
...
public class JWTAuthenticationFilter extends AbstractAuthenticationProcessingFilter {
  ...
  protected void successfulAuthentication(HttpServletRequest request, HttpServletResponse response, FilterChain chain,
    ...
    UserDetailsImpl userDetailsImpl = (UserDetailsImpl) authResult.getPrincipal();
    response.getWriter().write(new ObjectMapper().writeValueAsString(new UserMapperImpl().toDTO(userDetailsImpl.user())));
  }
